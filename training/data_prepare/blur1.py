...
class VideoHandDetector:
    """基于 MediaPipe Tasks API 的视频/摄像头人手检测器，支持裁剪手部居中或背景模糊化（输出无标注）"""
    ...
    def blur_background_hand(self, frame: np.ndarray, hand_landmarks_list: List,
                             margin: int = 50, blur_kernel_size: int = 51, blur_sigma: float = 30.0) -> np.ndarray:
        """
        将手部区域保持清晰，背景进行高斯模糊（支持边缘羽化）。

        Args:
            frame: 原始 BGR 图像
            hand_landmarks_list: 检测到的所有手部的关键点列表（每个元素是 NormalizedLandmark 列表）
            margin: 包围框四周额外保留的像素数（用于扩展手部区域）
            blur_kernel_size: 高斯模糊核大小（奇数，越大越模糊）
            blur_sigma: 高斯模糊标准差
        ...
        # 对掩码进行高斯模糊，得到羽化权重（边缘过渡更自然）
        # 注意：模糊核大小必须为正奇数
        if blur_kernel_size % 2 == 0:
            blur_kernel_size += 1
        soft_mask = cv2.GaussianBlur(mask, (blur_kernel_size, blur_kernel_size), blur_sigma)
        ...
        # 将 soft_mask 扩展为三通道，用于融合
        soft_mask_3c = np.stack([soft_mask, soft_mask, soft_mask], axis=-1)

        # 融合：手部区域使用原图，背景使用模糊图
        result = (frame * soft_mask_3c + blurred * (1 - soft_mask_3c)).astype(np.uint8)
    ...
    # ---------- 修改后的处理函数，增加 blur_background 选项 ----------
    def process_video(self,
        ...
        """
        处理视频文件，可进行裁剪或背景模糊。
        注意：crop_hand 和 blur_background 不可同时为 True，若同时为 True 则默认使用 blur_background。
        """
...
if __name__ == "__main__":
    ...
    # 处理互斥选项：若同时指定 --crop 和 --blur，已在函数内部处理为默认使用 blur
    crop = args.crop
    blur = args.blur
