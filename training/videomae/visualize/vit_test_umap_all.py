# vit_umap_visualization.py
# 使用 UMAP 降维（2D）可视化 VideoMAE 特征
# - 图1：按 8 个动作类别着色（带图例）
# - 图2：按 4 个不同视角着色（带图例）
# - 额外：每个类别一张子图，区分四个视角
...
# UMAP 参数
n_components = 2
n_neighbors = 50          # 局部邻域大小，越小越关注局部结构
min_dist = 0.1            # 嵌入点之间的最小距离
#metric = 'euclidean'      # 距离度量cosine或euclidean
metric = 'cosine' 
random_state = 42
...
# ==================== 绘图函数 ====================
def plot_umap_by_class(features_2d, labels, save_path='umap_by_class.png'):
    ...
    plt.title('UMAP of [CLS] Features (by Action Class)', fontsize=14)
    plt.xlabel('UMAP1')
    plt.ylabel('UMAP2')
    plt.tight_layout()
...
def plot_umap_by_view(features_2d, views, view_color_dict, save_path='umap_by_view.png'):
    plt.figure(figsize=(8, 6))
    ...
    plt.title('UMAP of [CLS] Features (by View)', fontsize=14)
    plt.xlabel('UMAP1')
    plt.ylabel('UMAP2')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
...
def plot_per_class_separate_umap(features_2d, labels, views, class_names, view_color_dict, out_dir='./'):
    for c in range(len(class_names)):
        ...
        for v in unique_views_in_class:
            ...
            plt.scatter(features_2d[mask, 0], features_2d[mask, 1],
                        c=color, label=v, alpha=0.6, s=20)
        plt.title(f'Class {class_names[c]} (UMAP)', fontsize=12)
        plt.legend(title="View", fontsize=8, title_fontsize=9)
        plt.xlabel('UMAP1')
        plt.ylabel('UMAP2')
        plt.tight_layout()
        save_path = os.path.join(out_dir, f'umap_class_{class_names[c]}.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"已保存: {save_path}")
...
def main():
    ...
    print(f"UMAP 完成，嵌入形状: {features_2d.shape}")

    # 7. 绘图
    plot_umap_by_class(features_2d, all_labels, save_path='umap_by_class.png')
    plot_umap_by_view(features_2d, all_views, view_colors, save_path='umap_by_view.png')
    plot_per_class_separate_umap(features_2d, all_labels, all_views, class_names,
                                 view_colors, out_dir='./')
