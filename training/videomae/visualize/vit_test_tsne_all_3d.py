# vit_tsne_3d_visualization.py
# 使用 t-SNE 降维到 3D，可视化 VideoMAE 特征
...
perplexity = 30
random_state = 42
init = 'random'          # 或 'random'
max_iter = 1500

n_samples_per_class = None   # 每类最多样本数，t-SNE 大数据量会很慢，建议采样
...
def plot_per_class_3d(features_3d, labels, views, class_names, view_color_dict, out_dir='./'):
    for c in range(len(class_names)):
        ...
        for v in unique_views:
            ...
            ax.scatter(features_3d[mask, 0], features_3d[mask, 1], features_3d[mask, 2],
                       c=color, label=v, alpha=0.6, s=15)
        ax.set_title(f'Class {class_names[c]} (t-SNE 3D)', fontsize=12)
        ax.legend(title="View", fontsize=8)
        ax.set_xlabel('t-SNE1')
        ax.set_ylabel('t-SNE2')
        ax.set_zlabel('t-SNE3')
        plt.tight_layout()
        save_path = os.path.join(out_dir, f'tsne3d_class_{class_names[c]}.png')
...
def main():
    ...
    if n_samples_per_class is not None:
        ...
        print(f"Sampled to {len(dataset)} samples (t-SNE may be slow with too many points)")
    else:
        dataset = full_dataset
        print("Warning: using all samples, t-SNE will be very slow!")
    ...
    # t-SNE
    print("Running t-SNE to 3D (this may take a while)...")
    ...
    plot_3d_scatter(features_3d, labels,
                    title='t-SNE 3D by Action Class',
                    filename='tsne3d_by_class.png',
                    is_class=True)
    plot_3d_scatter(features_3d, views,
                    title='t-SNE 3D by View',
                    filename='tsne3d_by_view.png',
                    is_class=False, view_color_dict=view_colors)
    plot_per_class_3d(features_3d, labels, views, class_names, view_colors)
    
    print("All t-SNE 3D visualizations done.")
