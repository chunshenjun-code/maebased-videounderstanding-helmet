...
class TRTYOLO:
    def __init__(self, engine_path, conf_thres=0.5, iou_thres=0.5):
        ...
        for i in range(self.engine.num_io_tensors):
            ...
            if self.engine.get_tensor_mode(name) == trt.TensorIOMode.INPUT:
                ...
                self.input_dtypes[name] = trt.nptype(self.engine.get_tensor_dtype(name))
                self.input_shapes[name] = self.engine.get_tensor_shape(name)
            else:
                self.output_names.append(name)
                self.output_dtypes[name] = trt.nptype(self.engine.get_tensor_dtype(name))
        ...
        self.input_shape = self.input_shapes[self.input_name]  # (1, 3, H, W)
        self.output_dtype = self.output_dtypes[self.output_name]
        self.output_shape = self.output_shapes[self.output_name]  # (1, 25200, 85)
        
        # 确保当前线程有活动的 CUDA 上下文
        if cuda.Context.get_current() is None:
            # 在 GPU 0 上创建并激活一个新的上下文
            cuda.Device(0).make_context()

        # 分配 GPU 内存缓冲区
        input_size = trt.volume(self.input_shape) * np.dtype(self.input_dtype).itemsize
        self.input_buffer = cuda.mem_alloc(input_size)
        output_size = trt.volume(self.output_shape) * np.dtype(self.output_dtype).itemsize
        self.output_buffer = cuda.mem_alloc(output_size)

        # 将缓冲区地址绑定到上下文
        self.context.set_tensor_address(self.input_name, int(self.input_buffer))
        self.context.set_tensor_address(self.output_name, int(self.output_buffer))

        # 创建 CUDA 流（用于异步执行）
        self.stream = cuda.Stream()

        print(f"TensorRT 引擎加载成功，输入: {self.input_name} {self.input_shape}, 输出: {self.output_name} {self.output_shape}")

    def preprocess(self, img):
        """预处理：保持宽高比缩放 + 填充至模型输入尺寸，归一化到 [0,1]"""
        ...
        canvas = np.transpose(canvas, (2, 0, 1))  # CHW
    ...
    def infer(self, img):
        ...
        # 拷贝输入数据到 GPU
        cuda.memcpy_htod_async(self.input_buffer, input_tensor.ravel().astype(self.input_dtype), self.stream)
        ...
        boxes, scores, class_ids = self.postprocess(output, scale, pad_w, pad_h, img.shape[:2])
        return boxes, scores, class_ids

    def postprocess(self, output, scale, pad_w, pad_h, img_shape):
        """解析 YOLO 输出，应用 NMS，返回原图坐标系下的检测框"""
        output = output[0]  # (num_dets, 85)
...
class VideoHandDetector:
    ...
    def __init__(self, confidence_threshold: float = 0.5, model_path: Optional[str] = None,
                 yolo_weights: Optional[str] = "best_fp16.engine", yolo_conf: float = 0.5, yolo_imgsz: int = 640):
        """初始化手部检测器，并可选择加载 YOLO 模型（TensorRT 引擎）"""
        # ========== 原有手部模型初始化 ==========
        ...
        if yolo_weights is not None:
            self.yolo_model = TRTYOLO(yolo_weights, conf_thres=yolo_conf)
            print(f"TensorRT YOLO 引擎加载完成: {yolo_weights}")
    ...
    def _get_object_mask(self, frame: np.ndarray, target_ids: Union[int, List[int]]) -> np.ndarray:
        ...
        # TensorRT 推理
        boxes, scores, class_ids = self.yolo_model.infer(frame)
    ...
    def _apply_blur_with_mask(self, frame: np.ndarray, mask: np.ndarray,
        ...
        """
        根据掩码对图像应用统一背景模糊（清晰区域保持不变，背景统一模糊）。
        参数 margin 已弃用（仅为兼容旧接口）。
        """
        h, w = frame.shape[:2]
        if np.sum(mask) == 0:
            # 无掩码时直接返回原图
...
# ====== 主程序示例（与 blur5 完全相同的命令行接口） ======
if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="视频手部检测与处理工具（裁剪或统一背景模糊）")
    ...
    parser.add_argument("--blur", action="store_true",
                        help="启用手部区域保留、背景统一模糊（与 --crop 互斥，若同时启用则默认使用 blur）")
    parser.add_argument("--margin", type=int, default=50,
                        help="[已弃用，仅供接口兼容] 手部包围盒边距（像素，默认50）")
    ...
    parser.add_argument("--yolo_weights", type=str, default=None,
                        help="TensorRT 引擎文件路径（如 best_fp16.engine），启用后可保留指定物体清晰")
